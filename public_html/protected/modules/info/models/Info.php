...
class Info extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
            'parent'=>array(self::BELONGS_TO, 'Info', 'info_id'),
            'data'=>array(self::BELONGS_TO, 'Data', 'data_id'),
            'infos'=>array(self::HAS_MANY, 'Info', 'info_id'),
            'actualInfos'=>array(
                self::HAS_MANY,
                'Info',
                'info_id',
                'order'=>'actualInfos.todo_time DESC',
                'condition'=>"`actualInfos`.`progress`!='done'",
            ),
		);
	}
	...
	public function rules()
	{
		...
		return array(
			...
			array('data_id, info_id, user_id', 'numerical', 'integerOnly'=>true),
			array('progress', 'length', 'max'=>11),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, data_id, info_id, user_id, progress, todo_time', 'safe', 'on'=>'search'),
		);
	}
